...
def crop_mask(mask):
    ...
    bg = Image.new(im.mode, im.size, 0)             # 背景颜色是0！！
    diff = ImageChops.difference(im, bg)
...
def crop_image_by_mask(img, mask):
    ...
    try:
        ...
        # adjusted_bbox = (0, y_start, x_end - x_start, y_end)
        # # 使用调整后的 bbox 来裁剪图像
        # crop_img = img[adjusted_bbox[1]:adjusted_bbox[3], adjusted_bbox[0]:adjusted_bbox[2]].copy()
    except:
        crop_img = img.copy()
    return crop_img
