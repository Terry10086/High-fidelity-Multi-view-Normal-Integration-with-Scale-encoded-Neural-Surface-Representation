...
class PinholeCamera:
    ...
    def build(self, device, H, W, resolution_level=1):
        # tx = torch.linspace(0, self.width - 1, int(self.width / resolution_level))
        # ty = torch.linspace(0, self.height - 1, int(self.height / resolution_level))
        # x, y = torch.meshgrid(ty, tx)

        x, y = torch.meshgrid(
            torch.arange(self.width/ resolution_level, device=device),
            torch.arange(self.height/ resolution_level, device=device),
            indexing="xy",
        )
        directions = F.pad(
            torch.stack(
                [
                    ...
                    (y - self.K[1, 2]) / self.K[1, 1] * self.sign_z,
                ],
                dim=-1,
            ),
            (0, 1),
        ...
        # np.save('./exp/histogram.npy',ray_cos)

        return origins, directions, radiis, ray_cos
